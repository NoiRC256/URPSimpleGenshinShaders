...
half3 ShadeEmissionDefaultMethod(ToonSurfaceData surfaceData, LightingData lightingData, Light light, bool isAdditionalLight)
{
	half3 N = lightingData.normalWS;
	half3 L = light.direction;
	half3 V = lightingData.viewDirectionWS;
	...
	// ====== Module Start: Rim Light ======
	half ndlWrapped = NoL * 0.5 + 0.5;
	half3 ramp = CalculateRamp(ndlWrapped);
	ramp *= CustomFaceShade(surfaceData, lightingData, light, isAdditionalLight);
	// rimMask if not using custom rim light lightmask;
	half3 rimMask = half3(1, 1, 1);

	half ndv = 1 - max(0, dot(V, N));
	half rim = smoothstep(rimMin, rimMax, ndv);
	if (useRimLight == 1) 
	{
	...
	emission.rgb += useRimLight ? rimMask.rgb * rim * _RimColor.rgb : 0;
...
half3 ShadeEmission(ToonSurfaceData surfaceData, LightingData lightingData, Light light)
{
    //you can switch to ShadeEmissionYourMethod(...) !
    return ShadeEmissionDefaultMethod(surfaceData, lightingData, light, false); 
}
